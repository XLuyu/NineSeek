...
def score_v0(kv):
    if kv['market_per_year']>0:
        for k,mpy in [(2e3,20),(2e4,40),(1e5,60),(1e6,80),(1e8,100)]:
            if kv['market_per_year']<=k: break
        co = {"1":100,"1＜顺位≤3":80,"顺位＞3":60,"三类医疗器械":40,"改良创新药":40,"仿制药或二类及以下医疗器械":20}[kv['china_order']]
        go = {"1":100,"1＜顺位≤3":80,"顺位＞3":60,"三类医疗器械":40,"改良创新药":40,"仿制药或二类及以下医疗器械":20}[kv['global_order']]
        return 0.6*mpy+0.3*co+0.1*go
    else: 
        cm = {"新兴市场":80,"快速成长市场":100,"成熟市场":60,"边缘市场":40}[kv['china_market_type']]
        gm = {"新兴市场":80,"快速成长市场":100,"成熟市场":60,"边缘市场":40}[kv['EU_US_market_type']]
        for k,tms in [(20,100),(40,80),(60,60),(80,40),(100,20)]:
            if kv['top2_marketshare']<=k: break
